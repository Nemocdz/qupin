...
- (void)creatView{
    ...
    [self.contentView addSubview:self.taskTitileLabel];
...
- (void)setLayout{
    ...
    [self.taskTitileLabel mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.moneyLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.taskTitileLabel.mas_left);
    }];
    
    [self.timeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(self.cellImageView.mas_bottom);
        make.right.equalTo(self.taskTitileLabel.mas_right);
    }];
    
}

- (void)setItem:(QPTaskListItem *)item{
    self.taskTitileLabel.text = item.taskTitile;
...
- (UILabel *)taskTitileLabel{
    if (!_taskTitileLabel) {
        _taskTitileLabel = [UILabel new];
        _taskTitileLabel.font = [UIFont systemFontOfSize:18.f];
        _taskTitileLabel.numberOfLines = 0;
        _taskTitileLabel.textColor = QP_TASKCELLTITLEBLACK;
    }
    return _taskTitileLabel;
}
